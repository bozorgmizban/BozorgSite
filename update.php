...
<head>
	<meta charset="utf-8">
	<title>بزرگ سایت <?php echo UPDATER_VERSION ?></title>
...
<body dir="rtl">
	...
	<div id="container">
		...
		<section id="configuration">
			<?php
				...
				if(file_exists($filenamepost))
				{
					...
					{
						echo Html::p( array('class'=>'pass', 'content'=>'بانک داده به روز است: '.FILE_XML_POSTS) );
						@unlink($filenamepost);
					}
					else
						echo Html::p( array('class'=>'pass', 'content'=>'خطا - بانک داده به روز شده: '.FILE_XML_POSTS) );
				}
				...
				foreach($posts_files as $file_old)
				{
					...
					if(count($explode)==11)
					{
						...
						echo Html::p( array('class'=>'pass', 'content'=>'نام فایل تغییر داده شده: '.$file_old.' => '.$filename) );
					}
				}
				...
				if(!file_exists(FILE_XML_NOTIFICATIONS))
				{
					...
					echo Html::p( array('class'=>'pass', 'content'=>'فایل ساخته شد: '.FILE_XML_NOTIFICATIONS) );
				}
				...
				if(!file_exists(FILE_XML_USERS))
				{
					...
					echo Html::p( array('class'=>'pass', 'content'=>'فایل ساخته شد: '.FILE_XML_USERS) );
				}
				...
				if(!file_exists(FILE_XML_TAGS))
				{
					...
					echo Html::p( array('class'=>'pass', 'content'=>'فایل ساخته شد: '.FILE_XML_TAGS) );
				}
				else
				{
    				...
    				if ($count > 0)
    				{
        				echo Html::p( array('class'=>'pass', 'content'=>'بانک داده به روز شد: '.FILE_XML_TAGS) );
    				}
				}
				...
				if(!file_exists(FILE_XML_PAGES))
				{
					...
					echo Html::p( array('class'=>'pass', 'content'=>'فایل ساخته شد: '.FILE_XML_PAGES) );
				...
				if($obj->asXml( FILE_XML_CONFIG ))
					echo Html::p( array('class'=>'pass', 'content'=>'بانک داده به روز شد: '.FILE_XML_CONFIG) );
				else
					echo Html::p( array('class'=>'pass', 'content'=>'خطا - به روز رسانی بانک داده: '.FILE_XML_CONFIG) );
				...
				echo Html::p( array('class'=>'pass', 'content'=>'بانک داده به روز شد: '.FILE_XML_COMMENTS) );
				...
				echo Html::p( array('class'=>'pass', 'content'=>'دسته بندی به روز شد...') );
		...
		<footer>
			<p><span class="blog-footer"><i class="fas fa-eye"></i> توسعه داده شد توسط <i class="fas fa-server"></i><a href="https://BozorgMizban.ir">بزرگ میزبان</a></span></p>
		</footer>
