...
/*
 * For 1-Click Setup
 * This are the POST variables to install Bzrg without the default form.
...
if( file_exists('content/private') || file_exists('content/public') )
	exit('<title>نصب موفق</title><h1 align="center" dir="rtl">بزرگ سایت قبلا نصب شده، برای   <a href="update.php">به روز رسانی</a> کلیک کنید.</h1>');
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' )
{
	...
	$obj->addChild('footer',				$_LANG['POWERED_BY_BzrgSite']);
	...
	$node->addAttribute('slug', $_LANG['UNCATEGORIZE']);
	$node->addAttribute('position', 1);
	$node = $obj->addChild('category', '');
	$node->addAttribute('id',1);
	$node->addAttribute('name', $_LANG['NEWS']);
	$node->addAttribute('slug', $_LANG['NEWSCAT']);
	$node->addAttribute('position', 2);
	$node = $obj->addChild('category', '');
	$node->addAttribute('id',2);
	$node->addAttribute('name', $_LANG['ARTICLES']);
	$node->addAttribute('slug', $_LANG['ARTICLE']);
	...
	$content = Text::replace_assoc(
			array(
				...
				'{{WEB_LINK}}'=>'<a target="_blank" href="https://Bzrg.ir">بزرگ میزبان</a>',
				'{{LINKDIN_LINK}}'=>'<a target="_blank" href="https://linkedin.com/company/bozorg-mizban">لینک دین</a>',
				'{{EMAIL_LINK}}'=>'<a target="_blank" href="mailto:Mail@Bzrg.ir">پست الکترونیک</a>'
			),
			$content
	);
	...
	foreach($plugins as $plugin)
	{
		...
		if( @!include(PATH_PLUGINS.$plugin.'/languages/'.$_GET['language'].'.bit') )
			include(PATH_PLUGINS.$plugin.'/languages/fa_IR.bit');
...
<body dir="rtl">
	...
	<div id="container">
		...
		<header id="head">
			<?php
				echo Html::h1( array('content'=>$_LANG['WELCOME_TO_BzrgSite']) );
			?>
		</header>
		...
		<section id="complete">
			<?php
				...
				echo Html::p( array('content'=>$_LANG['INSTALLATION_LINE5'].' <a href="http://bzrg.ir">http://bzrg.ir</a>') );
			?>
		</section>
		...
		<section id="dependencies">
			...
			<?php
				...
				echo Html::div_open( array('class'=>'dependency') );
					echo Html::link( array('class'=>'description', 'content'=>$_LANG['WRITING_TEST_ON_CONTENT_DIRECTORY'], 'href'=>'http://BozorgSite.ir/Install', 'target'=>'_blank') );
		...
		<section id="configuration">
			...
			<?php
				echo Html::form_open( array('id'=>'js_form', 'name'=>'form', 'method'=>'post') );
					...
					echo Html::select( array('id'=>'js_language', 'name'=>'language'), $languages, isset($_GET['language'])?$_GET['language']:'fa_IR');

 					echo Html::div_close();

					echo Html::label( array('content'=>$_LANG['BLOG_TITLE']) );
					echo Html::input( array('id'=>'js_name', 'name'=>'name', 'type'=>'text', 'autocomplete'=>'off', 'maxlength'=>'254', 'value'=>'', 'placeholder'=>'بزرگ سایت') );

					echo Html::label( array('content'=>$_LANG['BLOG_SLOGAN']) );
					echo Html::input( array('id'=>'js_slogan', 'name'=>'slogan', 'type'=>'text', 'autocomplete'=>'off', 'maxlength'=>'254', 'value'=>'', 'placeholder'=>'بزرگ سایت، انتخاب بزرگان') );

					echo Html::label( array('content'=>$_LANG['ADMINISTRATOR_USERNAME'].'*') );
					echo Html::input( array('id'=>'js_username', 'name'=>'username', 'type'=>'text', 'autocomplete'=>'off', 'maxlength'=>'254', 'value'=>'', 'placeholder'=>'نام کاربری مدیر سایت') );

					echo Html::label( array('content'=>$_LANG['ADMINISTRATOR_PASSWORD'].'*') );
					echo Html::input( array('id'=>'js_password', 'name'=>'password', 'type'=>'text', 'autocomplete'=>'off', 'maxlength'=>'254', 'value'=>'', 'placeholder'=>'رمز کاربری مدیر سایت') );

					echo Html::label( array('content'=>$_LANG['ADMINISTRATOR_EMAIL'].'*') );
					echo Html::input( array('id'=>'js_email', 'name'=>'email', 'type'=>'text', 'autocomplete'=>'off', 'value'=>'', 'placeholder'=>'آدرس ایمیل فعال') );
		...
		<footer>
			<p><a href="http://BozorgSite.ir">بزرگ سایت <?php echo NIBBLEBLOG_VERSION ?></a> ©<?php echo date('Y') ?> | توسعه داده شده توسط <a href="http://BozorgMizban.ir">بزرگ میزبان</a></p>
		</footer>
